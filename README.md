...
After running all the cells, the `osm.sumocfg` file must be changed so that one of the `<route-files>` parameters finaltrips.rou.xml (or the chosen name for the `final_trips_file_name` variable).
